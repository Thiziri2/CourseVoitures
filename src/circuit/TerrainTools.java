...
public class TerrainTools {
	...
	public static BufferedImage imageFromCircuit(Terrain[][] track) {
		BufferedImage img = new BufferedImage(track[0].length, track.length, BufferedImage.TYPE_INT_ARGB);
		...
		for(int i=0;i<track.length;i++) {
 			for(int j=0;j<track[0].length;j++) {
 				...
 				//g.drawLine(j, i, j, i);
 				img.setRGB(j,i,terrainToRGB(track[i][j]).getRGB() ); //inverser l'image
 			}
 		}
 	    return img;
    	}
