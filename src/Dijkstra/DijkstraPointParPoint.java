...
public class DijkstraPointParPoint extends Dijkstra{
	Circuit cir;
	private Vecteur cible;
	double [][] distance;
	Comparator<Vecteur> comp;
	PriorityBlockingQueue <Vecteur> Q;
	//ArrayList<Vecteur> Q = new ArrayList<>();

	public DijkstraPointParPoint(Circuit cir, Vecteur cible){
		super(cir);
		this.cible = cible;
		this.cir = cir;
		distance = new double [cir.getHeight()] [cir.getWidth()];
		//initialisation du tableau de distance
		for(int i=0; i<distance.length; i++){
			for(int j=0; j<distance[0].length; j++){
				if(cible.x == i && cible.y == j){
					distance[i][j] = 0;
				}
				else{
	...
	private void neighborUpdate(Vecteur v){
		...
		for(int i =-1; i<= 1;i++){
			for(int j = -1; j<= 1;j++){
				...
				if(distance[x][y]==0){
					if((new Vecteur(i,j)).prodScal(cir.getDirectionArrivee()) > 0){
						continue;
					}
				}
